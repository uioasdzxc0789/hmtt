<template>
  <div class="article-container">
    ...
    <!-- /底部区域 -->
    <!-- 在vue组件中，把一些和当前组件无关的不属于文档流中的东西放最后 -->
    <!-- 每次关闭，组件销毁，每次打开，组件重新创建，mounted就会再执行一次 -->
    <van-popup v-model="addCommentShow" position="bottom"
      ...
      ></AddComment
    ></van-popup>
    <van-popup v-model="isReplayShow" position="bottom" style="height: 100vh">
      <ReplayComment
        :comment="comment"
...
export default {
  ...
  async created () {
    try {
      ...
      // 可选链，等价于err.response && err.response.status
      if (err.response?.status === 404) { this.is404Error = true }
    }
